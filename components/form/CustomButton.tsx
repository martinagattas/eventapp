...
export const CustomButton: FC<CustomButtonI> = ({ children, variant, customColor, type, disabled, href, onClick }: CustomButtonI) => {
  const buttonVariant = variant === 'outlined' ? s[`button-outline`] : s.button;

  const buttonClass = `${buttonVariant} ${s[customColor]}`;
